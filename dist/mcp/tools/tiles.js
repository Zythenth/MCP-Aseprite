...
export function registerTileTools(server, dispatcher, state) {
    server.tool("list_tilesets", "Lists tilesets, tile dimensions, base index, and tile count.", {}, async () => {
        try {
            return bridgeToolResult(await dispatcher.send("list_tilesets", {}, 5000), state);
        }
        catch (error) {
    ...
    server.tool("delete_tileset", "Deletes a tileset by zero-based tilesetIndex. Requires confirmation.", {
        ...
        if (!params.confirm)
            return confirmationError("delete_tileset");
        try {
            return bridgeToolResult(await dispatcher.send("delete_tileset", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("get_tile", "Gets one tile's metadata and PNG preview.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("get_tile", params, 10000), state, true);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("set_tile_pixels", "Atomically paints pixels in one reusable tile image.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("set_tile_pixels", params, 15000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("create_tilemap_layer", "Creates a tilemap layer associated with an existing tileset.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("create_tilemap_layer", params, 15000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("get_tilemap", "Reads a tilemap cel as tile indexes and flip flags without expanding it to pixels.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("get_tilemap", params, 10000), state);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("set_tiles", "Atomically writes tile references and X/Y/diagonal flip flags into a tilemap cel.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("set_tiles", params, 15000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
