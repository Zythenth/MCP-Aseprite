 ...
 */
import { z } from "zod";
import { MAX_FINDINGS, } from "../animationWorkflowState.js";
...
const targetSelectionSchema = z
    ...
    .strict();
export function registerWorkflowTools(server, _dispatcher, stateTracker, workflowState) {
    const syncState = () => {
        workflowState.setContext(stateTracker.getSessionId(), stateTracker.getActiveSprite()?.filename || null, stateTracker.getRevision());
    };
    // 1. create_animation_workflow
    server.tool("create_animation_workflow", "Initializes a new animation workflow session requiring a connected bridge and active sprite.", {
    ...
    server.tool("get_animation_workflow", "Returns the current compact summary and status of the active animation workflow.", {}, async () => {
        try {
            syncState();
    ...
    server.tool("record_reference_analysis", "Records formal visual analysis of a reference image with exact character and design breakdown.", {
    ...
    server.tool("record_animation_plan", "Records the expanded animation plan (action, discriminated target, timing, planned key poses, arcs, rules, criteria).", {
    ...
    // 5. mark_key_pose
    server.tool("mark_key_pose", "Marks a timeline frame as a key pose from the recorded plan. Role and name are inferred directly from the plan.", {
    ...
    // 6. review_key_poses
    server.tool("review_key_poses", "Reviews key poses against current revision and registered previewId. Accepts structured findings.", {
    ...
    const selfReviewCategoryItemSchema = z
        ...
        .strict();
    server.tool("record_self_review", "Records author self-review covering all 22 exact categories. Uses preview's temporalHash automatically.", {
        previewId: z.string().min(1).max(128).describe("Preview ID previously rendered via render_animation_preview"),
        revision: z.number().int().positive().describe("Current sprite revision (must match current Bridge revision)"),
    ...
    server.tool("submit_animation_qa", "Submits independent QA evaluation. Reviewer must be different from author and confirm independence.", {
    ...
    server.tool("resolve_animation_finding", "Resolves or accepts an animation finding with non-empty justification. Critical severity cannot be accepted.", {
    ...
    server.tool("validate_animation_completion", "Audits all animation workflow completion gates without mutating editor or blocking generic operations.", {}, async () => {
        try {
            syncState();
            const completion = workflowState.validateCompletion();
            return {
    ...
    server.tool("reset_animation_workflow", "Destructively resets the active animation workflow metadata. Requires confirm: true.", {
