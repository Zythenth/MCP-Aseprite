...
export function registerSelectionTools(server, dispatcher, state) {
    server.tool("get_selection", "Gets the persistent sprite selection bounds and origin.", {}, async () => {
        try {
            return bridgeToolResult(await dispatcher.send("get_selection", {}, 5000), state);
        }
        catch (error) {
    ...
    server.tool("set_selection", "Replaces, adds, subtracts, or intersects a persistent rectangular selection.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("set_selection", params, 5000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("clear_selection", "Clears the persistent sprite selection.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("clear_selection", params, 5000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("invert_selection", "Inverts the persistent selection inside the sprite canvas.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("invert_selection", params, 5000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
