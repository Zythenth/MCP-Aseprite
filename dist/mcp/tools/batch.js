import { z } from "zod";
import { MAX_PIXELS_BATCH, MAX_ANIMATION_BATCH_OPERATIONS, MAX_ANIMATION_BATCH_FRAMES, MAX_ANIMATION_BATCH_PAYLOAD_BYTES, } from "../../config.js";
...
export function registerBatchTools(server, dispatcher, stateTracker) {
    server.tool("batch_animation_edits", "Executes an atomic batch of animation edits across frames and cels in a single transaction.", {
        operations: batchOperationsArraySchema.describe("Atomic array of animation edit operations (1..64)"),
        returnPreview: z
            .boolean()
            .optional()
            .default(false)
        ...
        try {
            ...
            if (payloadBytes > MAX_ANIMATION_BATCH_PAYLOAD_BYTES) {
                throw new Error(`Batch payload size (${payloadBytes} bytes) exceeds safety limit of ${MAX_ANIMATION_BATCH_PAYLOAD_BYTES} bytes (4 MiB).`);
            }
            ...
            if (totalPixelsOrPoints > MAX_PIXELS_BATCH) {
                throw new Error(`Total pixels/points across batch operations (${totalPixelsOrPoints}) exceeds safety limit of ${MAX_PIXELS_BATCH}.`);
            }
            if (touchedFrames.size > MAX_ANIMATION_BATCH_FRAMES) {
                throw new Error(`Batch affects ${touchedFrames.size} unique frames, exceeding limit of ${MAX_ANIMATION_BATCH_FRAMES}.`);
            }
            const res = await dispatcher.send("batch_animation_edits", params, 30000);
