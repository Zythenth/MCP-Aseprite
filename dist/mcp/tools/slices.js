...
export function registerSliceTools(server, dispatcher, state) {
    server.tool("list_slices", "Lists slices with canvas bounds, local nine-patch center, local pivot, color, and data.", {}, async () => {
        try {
            return bridgeToolResult(await dispatcher.send("list_slices", {}, 5000), state);
        }
        catch (error) {
    ...
    server.tool("get_slice", "Gets one slice by its exact name.", { name: z.string().min(1).max(128) }, async (params) => {
        try {
            return bridgeToolResult(await dispatcher.send("get_slice", params, 5000), state);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("create_slice", "Creates a slice with optional local pivot and local nine-patch center.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("create_slice", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("update_slice", "Updates or clears slice bounds, pivot, nine-patch center, color, data, or name.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("update_slice", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("delete_slice", "Deletes a named slice. Requires confirmation.", {
        ...
        if (!params.confirm)
            return confirmationError("delete_slice");
        try {
            return bridgeToolResult(await dispatcher.send("delete_slice", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
