...
export function registerCelTools(server, dispatcher, state) {
    server.tool("get_cel", "Gets cel metadata, including canvas bounds, position, opacity, and linked frames.", {
        ...layerSelector,
        ...frameSelector,
    }, async (params) => {
        try {
            return bridgeToolResult(await dispatcher.send("get_cel", params, 5000), state);
        }
        catch (error) {
    ...
    server.tool("create_cel", "Creates an explicit image cel without changing the active layer or frame.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("create_cel", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("delete_cel", "Deletes a cel while preserving its layer and frame. Requires confirmation.", {
        ...
        if (!params.confirm)
            return confirmationError("delete_cel");
        try {
            return bridgeToolResult(await dispatcher.send("delete_cel", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("set_cel_position", "Moves a cel using either absolute x/y coordinates or relative dx/dy offsets.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("set_cel_position", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("set_cel_opacity", "Sets a cel's opacity from 0 through 255.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("set_cel_opacity", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("link_cel", "Creates a target cel in the same image layer that shares image data with a source cel.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("link_cel", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
    ...
    server.tool("unlink_cel", "Breaks a cel link by cloning its image into an independent image.", {
        ...
        try {
            return bridgeToolResult(await dispatcher.send("unlink_cel", params, 10000), state, params.returnPreview);
        }
        catch (error) {
            return bridgeToolError(error);
        }
