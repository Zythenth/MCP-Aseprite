...
function collectClusters(grid) {
    ...
    for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
            ...
            const key = `${x},${y}`;
            if (!isOpaque(color) || visited.has(key))
                continue;
            const pixels = [];
            const queue = [{ x, y }];
            visited.add(key);
            for (let index = 0; index < queue.length; index += 1) {
                ...
                for (const [dx, dy] of directions) {
                    ...
                    if (visited.has(neighborKey) || normalizeColor(grid[ny][nx]) !== color)
                        continue;
...
function putCluster(output, cluster, offsetX, offsetY) {
    for (const point of cluster.pixels) {
        const x = point.x + offsetX;
        const y = point.y + offsetY;
        if (y >= 0 && y < output.length && x >= 0 && x < output[0].length)
            output[y][x] = cluster.color;
    }
}
function matchClusters(source, target) {
...
export function buildClusterTween(source, target, progress, easing = "ease_in_out") {
    ...
    for (const cluster of sourceClusters) {
        if (!pairedSource.has(cluster) && amount < 0.5)
            putCluster(output, cluster, 0, 0);
    }
    for (const cluster of targetClusters) {
        if (!pairedTarget.has(cluster) && amount >= 0.5)
            putCluster(output, cluster, 0, 0);
    }
...
function rasterLine(output, color, fromX, fromY, toX, toY) {
    let x = fromX;
    let y = fromY;
...
export function buildSmearFrame(source, target, stretch) {
    if (source.length === 0 || source.length !== target.length || source[0].length !== target[0].length) {
    ...
    if (!sourceBounds || !targetBounds)
        throw new Error("Smear frames require opaque pixels in both key poses.");
    ...
    if (magnitude < 0.5)
        throw new Error("Smear frames require visible movement between key poses.");
