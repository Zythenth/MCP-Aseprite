...
export interface MockTileset {
  ...
  tileHeight: number;
  baseIndex: number;
  tiles: Array<{ pixels: Uint32Array; color: string | null; data: string }>;
}
...
function alphaComposite(dstColor: number, srcColor: number, opacity = 255): number {
  ...
  if (outA <= 0) return 0;
  return packRgba(
    Math.round((src.r * sa + dst.r * da * (1 - sa)) / outA),
    Math.round((src.g * sa + dst.g * da * (1 - sa)) / outA),
    Math.round((src.b * sa + dst.b * da * (1 - sa)) / outA),
    Math.round(outA * 255)
  );
}
...
function packTileReference(index: number, xFlip = false, yFlip = false, diagonalFlip = false): number {
...
function unpackTileReference(value: number): { tileIndex: number; xFlip: boolean; yFlip: boolean; diagonalFlip: boolean } {
  return {
...
function encodePixelBufferPng(width: number, height: number, pixels: Uint32Array): string {
...
export class MockAsepriteEngine {
  ...
  public tilemaps = new Map<string, MockTilemapCel>();
  ...
  public reset(width = 32, height = 32): void {
    ...
    this.tags = [];
    this.slices = [];
    this.selectionPixels = new Set<number>();
    this.tilesets = [];
    this.tilemaps = new Map<string, MockTilemapCel>();
  ...
  private getLayerPixel(layer: MockLayer, frameNumber: number, canvasX: number, canvasY: number): { color: number; celOpacity: number } {
    if (layer.isTilemap) {
      ...
      if (!map || !tileset) return { color: 0, celOpacity: 255 };
      const relativeX = canvasX - map.origin.x;
      const relativeY = canvasY - map.origin.y;
      if (relativeX < 0 || relativeY < 0) return { color: 0, celOpacity: 255 };
      const cellX = Math.floor(relativeX / tileset.tileWidth);
      const cellY = Math.floor(relativeY / tileset.tileHeight);
      if (cellX >= map.width || cellY >= map.height) return { color: 0, celOpacity: 255 };
      const reference = unpackTileReference(map.values[cellY * map.width + cellX]);
      const tile = tileset.tiles[reference.tileIndex];
      if (!tile) return { color: 0, celOpacity: 255 };
      let x = relativeX % tileset.tileWidth;
      let y = relativeY % tileset.tileHeight;
      if (reference.diagonalFlip) [x, y] = [y, x];
      if (reference.xFlip) x = tileset.tileWidth - 1 - x;
      if (reference.yFlip) y = tileset.tileHeight - 1 - y;
      if (x < 0 || y < 0 || x >= tileset.tileWidth || y >= tileset.tileHeight) return { color: 0, celOpacity: 255 };
    ...
    if (targetLayer) {
      ...
      for (let y = 0; y < this.height; y++) {
        for (let x = 0; x < this.width; x++) {
          ...
          if (layerPixel.color === 0) continue;
  ...
  public resolveAnyLayer(params: Record<string, any>, nameKey = "layerName", indexKey = "layerIndex"): MockLayer {
    const byName = params[nameKey] === undefined
      ? undefined
      : this.layers.filter((layer) => layer.name === params[nameKey]);
    if (byName && byName.length === 0) throw new Error(`Layer '${params[nameKey]}' not found.`);
    if (byName && byName.length > 1) throw new Error(`Ambiguous layer name '${params[nameKey]}'.`);
    ...
    if (byName?.[0] && byIndex && byName[0] !== byIndex) throw new Error("Conflicting layer selectors.");
    const layer = byName?.[0] ?? byIndex ?? this.layers.find((item) => item.index === this.activeLayerIndex) ?? this.layers[0];
    if (!layer) throw new Error("No target layer available.");
    return layer;
  }

  private recordChange(entry: PixelJournalEntry): void {
    this.changeJournal.push(entry);
    if (this.changeJournal.length > MAX_CHANGE_JOURNAL_ENTRIES) this.changeJournal.shift();
  }
  ...
  private selectionBounds(): MockCelBounds | null {
    if (this.selectionPixels.size === 0) return null;
  ...
  public executeCommand(command: string, params: Record<string, any>): any {
    switch (command) {
      ...
      case "get_changes_since": {
        ...
        const refreshRequired = (reason: string) => ({
          changed: true,
          sinceRevision: since,
        ...
        const changes: PixelJournalEntry[] = [];
      ...
      case "create_group": {
        ...
        if (params.parentGroup) {
          ...
          if (!parent.isGroup) throw new Error("parentGroup must name a group layer.");
      ...
      case "get_cel": {
        ...
        if (!cel) return { success: true, hasCel: false, layer: layer.name, frameNumber: frame.frameNumber, revision: this.revision };
      ...
      case "create_cel": {
        ...
        if (this.cels.has(key)) throw new Error("Cel already exists at target layer and frame.");
      ...
      case "delete_cel": {
        if (params.confirm !== true) throw new Error("delete_cel requires confirm: true");
        const layer = this.resolveTargetLayer(params, true);
        const frame = this.resolveTargetFrame(params.frameNumber);
        const key = `${layer.index}:${frame.frameNumber}`;
        const cel = this.cels.get(key);
        if (!cel) throw new Error("Cel not found at target layer and frame.");
      ...
      case "set_cel_position": {
        ...
        if (hasAbsolute === hasRelative) throw new Error("Provide either x/y or dx/dy, but not both.");
      ...
      case "unlink_cel": {
        ...
        if (!isLinked) return { success: true, changed: false, linked: false, bounds: { ...cel.bounds }, revision: this.revision };
      ...
      case "move_layer_to_group": {
        ...
        if (params.parentGroup !== undefined) {
          const parent = this.resolveAnyLayer({ layerName: params.parentGroup });
          if (!parent.isGroup) throw new Error("Target parent is not a group.");
          let cursor: MockLayer | undefined = parent;
          while (cursor) {
            if (cursor === layer) throw new Error("Cannot move a group into itself or one of its descendants.");
      ...
      case "ungroup_layer": {
        ...
        if (!group.isGroup) throw new Error(`Layer is not a group: ${params.name}`);
        const children = this.layers.filter((layer) => layer.parentIndex === group.index);
        for (const child of children) child.parentIndex = group.parentIndex;
        this.layers.splice(this.layers.indexOf(group), 1);
        if (this.activeLayerIndex === group.index) this.activeLayerIndex = children[0]?.index ?? this.layers[0]?.index ?? 0;
        return this.finishMutation(params, {
      ...
      case "merge_down_layer": {
        if (params.confirm !== true) throw new Error("merge_down_layer requires confirm: true");
        ...
        if (siblingIndex <= 0) throw new Error("Cannot merge the bottom layer down.");
        const bottom = siblings[siblingIndex - 1];
        if (bottom.isGroup) throw new Error("Cannot merge an image layer into a group layer.");

        for (const frame of this.frames) {
          const bottomCel = this.cels.get(`${bottom.index}:${frame.frameNumber}`);
          const topCel = this.cels.get(`${top.index}:${frame.frameNumber}`);
          if (!bottomCel && !topCel) continue;
      ...
      case "flatten_layers": {
        if (params.confirm !== true) throw new Error("flatten_layers requires confirm: true");
        ...
        const composites = new Map<number, Uint32Array>();
        for (const frame of this.frames) composites.set(frame.frameNumber, this.getCompositeBuffer(frame.frameNumber));
        this.layers = [{
      ...
      case "get_slice": {
        ...
        if (matches.length === 0) throw new Error(`Slice not found: ${params.name}`);
        if (matches.length > 1) throw new Error(`Ambiguous slice name: ${params.name}`);
        return { success: true, slice: structuredClone(matches[0]), revision: this.revision };
      }

      case "create_slice": {
        if (this.slices.some((slice) => slice.name === params.name)) throw new Error(`Slice already exists: ${params.name}`);
        const bounds = { ...params.bounds } as MockCelBounds;
        const center = params.center ? { ...params.center } as MockCelBounds : null;
      ...
      case "update_slice": {
        ...
        if (!slice) throw new Error(`Slice not found: ${params.name}`);
        ...
        if (params.color !== undefined) slice.color = rgbaToHex(hexToRgba(params.color));
        if (params.data !== undefined) slice.data = params.data;
      ...
      case "delete_slice": {
        if (params.confirm !== true) throw new Error("delete_slice requires confirm: true");
        const index = this.slices.findIndex((slice) => slice.name === params.name);
        if (index < 0) throw new Error(`Slice not found: ${params.name}`);
      ...
      case "set_selection": {
        ...
        if (left >= right || top >= bottom) throw new Error("Selection rectangle does not intersect the sprite canvas.");
        const rectangle = new Set<number>();
        for (let y = top; y < bottom; y++) for (let x = left; x < right; x++) rectangle.add(y * this.width + x);
        const operation = params.operation ?? "replace";
        if (operation === "replace") this.selectionPixels = rectangle;
        else if (operation === "add") for (const pixel of rectangle) this.selectionPixels.add(pixel);
        else if (operation === "subtract") for (const pixel of rectangle) this.selectionPixels.delete(pixel);
        else if (operation === "intersect") this.selectionPixels = new Set([...this.selectionPixels].filter((pixel) => rectangle.has(pixel)));
        else throw new Error(`Unsupported selection operation: ${operation}`);
        const bounds = this.selectionBounds();
        return this.finishMutation(params, {
      ...
      case "clear_selection": {
        ...
        if (!oldBounds) return { success: true, changed: false, isEmpty: true, bounds: null, origin: { x: 0, y: 0 }, revision: this.revision };
      ...
      case "invert_selection": {
        ...
        for (let index = 0; index < this.width * this.height; index++) if (!this.selectionPixels.has(index)) inverted.add(index);
      ...
      case "create_tileset": {
        ...
        if (
          !Number.isInteger(params.tileWidth) || params.tileWidth < 1 || params.tileWidth > 1024
      ...
      case "delete_tileset": {
        if (params.confirm !== true) throw new Error("delete_tileset requires confirm: true");
        const tileset = this.tilesets[params.tilesetIndex];
        if (!tileset) throw new Error(`Tileset not found at index ${params.tilesetIndex}`);
        ...
        for (const layer of this.layers) {
          if (layer.tilesetIndex !== undefined && layer.tilesetIndex > params.tilesetIndex) layer.tilesetIndex--;
        }
        return this.finishMutation(params, { deletedTilesetIndex: params.tilesetIndex },
          { x: 0, y: 0, width: this.width, height: this.height }, "tilesets", this.activeFrameNumber, true);
      }
      ...
      case "get_tile": {
        ...
        if (!tile) throw new Error(`Tile not found at index ${params.tileIndex}`);
        return {
      ...
      case "set_tile_pixels": {
        const tileset = this.tilesets[params.tilesetIndex];
        if (!tileset) throw new Error(`Tileset not found at index ${params.tilesetIndex}`);
        if (params.tileIndex === 0) throw new Error("Tile 0 is reserved as the empty tile.");
        const tile = tileset.tiles[params.tileIndex];
        if (!tile) throw new Error(`Tile not found at index ${params.tileIndex}`);
        ...
        for (const pixel of params.pixels) {
          ...
          if (nextPixels[index] !== color) {
            nextPixels[index] = color; changed++;
            minX = Math.min(minX, pixel.x); minY = Math.min(minY, pixel.y);
            maxX = Math.max(maxX, pixel.x); maxY = Math.max(maxY, pixel.y);
          }
        }
        if (changed === 0) return { success: true, changed: false, pixelsChanged: 0, bounds: { x: 0, y: 0, width: 0, height: 0 }, revision: this.revision };
        ...
        if (params.returnPreview) result.pngBase64 = encodePixelBufferPng(tileset.tileWidth, tileset.tileHeight, tile.pixels);
        return result;
      }
      ...
      case "create_tilemap_layer": {
        ...
        if (params.parentGroup) {
          const parent = this.resolveAnyLayer({ layerName: params.parentGroup });
          if (!parent.isGroup) throw new Error("parentGroup is not a group.");
          parentIndex = parent.index;
        }
      ...
      case "get_tilemap": {
        ...
        if (!layer.isTilemap || layer.tilesetIndex === undefined) throw new Error("Target layer is not a tilemap.");
        const frame = this.resolveTargetFrame(params.frameNumber);
        const map = this.tilemaps.get(`${layer.index}:${frame.frameNumber}`);
        if (!map) return { success: true, hasCel: false, layer: layer.name, frameNumber: frame.frameNumber, revision: this.revision };
        const tiles = Array.from({ length: map.height }, (_, y) => Array.from({ length: map.width }, (_, x) =>
          unpackTileReference(map.values[y * map.width + x])
        ));
      ...
      case "set_tiles": {
        ...
        for (const item of params.tiles) {
          if (item.x < 0 || item.y < 0 || item.x >= map.width || item.y >= map.height) throw new Error("Tile cell outside tilemap bounds.");
          if (item.tileIndex < 0 || item.tileIndex >= tileset.tiles.length) throw new Error("tileIndex outside tileset bounds.");
          ...
          if (nextValues[index] !== value) {
            nextValues[index] = value; changed++;
            minX = Math.min(minX, item.x); minY = Math.min(minY, item.y);
            maxX = Math.max(maxX, item.x); maxY = Math.max(maxY, item.y);
          }
        }
        if (changed === 0) return { success: true, changed: false, tilesChanged: 0, bounds: { x: 0, y: 0, width: 0, height: 0 }, revision: this.revision };
        map.values = nextValues;
        if (!existingMap) this.tilemaps.set(mapKey, map);
        return this.finishMutation(params, {
      ...
      case "render_animation_gif": {
        ...
        for (const frameNumber of frameNumbers) this.resolveTargetFrame(frameNumber);
        ...
        const durationsMs = frameNumbers.map((frameNumber: number) =>
          Math.round(this.resolveTargetFrame(frameNumber).duration * 1000)
        );
        return {
          success: true,
          ...
          frameNumbers,
          durationsMs,
          totalDurationMs: durationsMs.reduce((sum: number, duration: number) => sum + duration, 0),
      ...
      case "export_sprite_sheet": {
        if (!params.outputPath || typeof params.outputPath !== "string") throw new Error("outputPath is required.");
        if (params.overwrite !== true && this.mockExistingFiles.has(params.outputPath)) {
          throw new Error(`File already exists and overwrite is false: ${params.outputPath}`);
        }
        ...
        let tag: typeof this.tags[number] | undefined;
        if (params.frameNumbers) {
          if (!Array.isArray(params.frameNumbers)) throw new Error("frameNumbers must be an array.");
          frameNumbers = params.frameNumbers.map((frameNumber: number) => this.resolveTargetFrame(frameNumber).frameNumber);
        } else if (params.tagName) {
          tag = this.tags.find((item) => item.name === params.tagName);
          if (!tag) throw new Error(`Tag not found: ${params.tagName}`);
          const forward = Array.from({ length: tag.to - tag.from + 1 }, (_, index) => tag!.from + index);
          if (tag.direction === "reverse") frameNumbers = [...forward].reverse();
          else if (tag.direction === "pingpong") frameNumbers = [...forward, ...forward.slice(1, -1).reverse()];
          else if (tag.direction === "pingpong_reverse") frameNumbers = [...forward].reverse().concat(forward.slice(1, -1));
          else frameNumbers = forward;
        } else {
        ...
        if (frameNumbers.length < 1 || frameNumbers.length > 256) throw new Error("Export range must contain between 1 and 256 frames.");
        if (params.layerNames) {
          for (const name of params.layerNames) {
            const layer = this.resolveAnyLayer({ layerName: name });
            if (layer.isGroup) throw new Error(`Group layers cannot be exported directly: ${name}`);
          }
        }
        const scale = params.scale ?? 1;
        const spacing = params.spacing ?? 0;
        if (!Number.isInteger(scale) || scale < 1 || scale > 32) throw new Error("scale must be an integer between 1 and 32.");
        if (!Number.isInteger(spacing) || spacing < 0 || spacing > 64) throw new Error("spacing must be an integer between 0 and 64.");
        ...
        if (!["horizontal", "vertical", "grid"].includes(layout)) throw new Error(`Invalid layout: ${layout}`);
        if (!Number.isInteger(columns) || columns < 1 || columns > 64) throw new Error("columns must be an integer between 1 and 64.");
        ...
        if (width * height > 67_108_864) throw new Error("Sprite sheet exceeds the 67,108,864 pixel safety limit.");
        this.mockExistingFiles.add(params.outputPath);
        return {
          success: true,
          outputPath: params.outputPath,
