...
export class MockAsepriteEngine {
  ...
  public exportFramePngBase64(frameNumber = this.activeFrameNumber): string {
    ...
    switch (command) {
      ...
      case "get_pixel_grid": {
        ...
        if (params.layerName) {
          ...
          if (!l) throw new Error(`Layer '${params.layerName}' not found.`);
          layerIdx = l.index;
        }
      ...
      case "set_pixels": {
        ...
        if (!layer || layer.isGroup) throw new Error("Cannot paint on invalid layer or group.");
      ...
      case "flood_fill": {
        ...
        if (targetColor === newColor) return { pixelsModified: 0, revision: this.revision };
