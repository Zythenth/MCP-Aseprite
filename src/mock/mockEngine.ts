...
export class MockAsepriteEngine {
  ...
  public tags: Array<{ name: string; from: number; to: number; color?: string; direction: "forward" | "reverse" | "pingpong" | "pingpong_reverse" }> = [];
  ...
  public executeCommand(command: string, params: Record<string, any>): any {
    switch (command) {
      ...
      case "export_sprite_sheet": {
        ...
        if (frameNumbers.length > 256) throw new Error("Export range must contain between 1 and 256 frames.");
