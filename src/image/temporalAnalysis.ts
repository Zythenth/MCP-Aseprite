...
import type {
  ...
  AnimationPlayback,
} from "../mcp/animationSelection.js";
...
export interface TemporalAnalysisThresholds {
  pixelDiffThreshold?: number; // default 0 (0-255)
  maxAdjacentChangeRatio?: number; // default 0.85 (0-1)
  maxCenterShift?: number; // default max(w, h) * 0.40
  maxAreaChangeRatio?: number; // default 0.60 (0-1)
  maxCelPositionJump?: number; // default max(w, h) * 0.50
  rigidTolerance?: number; // default 0.0 (0-1)
  maxContactPointDisplacement?: number; // default 0
  checkLoopContinuity?: boolean; // default true if playback loops
}
...
export interface PairwiseStepMetric {
  ...
  boundsA: { x: number; y: number; width: number; height: number } | null;
  boundsB: { x: number; y: number; width: number; height: number } | null;
...
export function computeCanonicalTemporalHash(
  width: number,
  height: number,
  frameNumbers: number[],
  durationsMs: number[],
  ...
  if (frameNumbers.length !== frames.length || durationsMs.length !== frames.length) {
    throw new Error(
      `Array length mismatch in temporal hash: frameNumbers (${frameNumbers.length}), durationsMs (${durationsMs.length}), frames (${frames.length}).`
    );
  }
...
function hexColorFromRgba(r: number, g: number, b: number, a: number): string {
...
function computeFrameStats(image: ImageBuffer): FrameStats {
...
function collectImageLayers(layers?: AnimationLayerInspection[]): AnimationLayerInspection[] {
  if (!layers) return [];
  const result: AnimationLayerInspection[] = [];
  function recurse(list: AnimationLayerInspection[]) {
    for (const l of list) {
...
export function analyzeAnimationTemporalPure(input: PureTemporalAnalysisInput): TemporalAnalysisResult {
  ...
  if (contactPoints) {
    ...
    for (const p of contactPoints) {
      ...
      const seenFramesInPoint = new Set<number>();
  ...
  const frameStats: FrameStats[] = frameBuffers.map((buf) => computeFrameStats(buf));
  const canvasArea = width * height;

  const perFrameMetrics: PerFrameMetric[] = frameStats.map((stats, idx) => ({
    sequenceIndex: idx + 1,
  ...
  const colorFrameCounts = new Map<string, number>();
  ...
  let warningCount = 0;
  let infoCount = 0;
  const flaggedRuleSet = new Set<string>();
  const findings: TemporalAnalysisFinding[] = [];

  function recordFinding(finding: TemporalAnalysisFinding): void {
    totalFindings++;
  ...
  const pairwiseDifferences: PairwiseStepMetric[] = [];
  ...
  if (contactPoints && contactPoints.length > 0) {
    for (const pt of contactPoints) {
      ...
      const posByFrame = new Map<number, ContactPointPosition>();
      for (const p of pt.positions) {
        posByFrame.set(p.frameNumber, p);
      }

      // Collect positions in order of appearance in playback sequence
      const contactSteps: Array<{ stepIndex: number; frameNumber: number; pos: ContactPointPosition }> = [];
      for (let k = 0; k < totalFrames; k++) {
        const fn = playback.frameNumbers[k];
  ...
  let loopSeamDiff: { changedPixels: number; changeRatio: number; centerShift: number } | undefined;
  ...
  for (const layer of imageLayers) {
    if (!layer.cels || layer.cels.length === 0) continue;

    const celByFrame = new Map<number, AnimationCelSummary>();
