import { BufferSizeMismatchError, InvalidDimensionError, type ImageBuffer } from "./png.js";
...
function validateImage(image: ImageBuffer): void {
  ...
  if (image.data.length < expected) throw new BufferSizeMismatchError(expected, image.data.length);
}

function requireMatchingFrames(frames: ImageBuffer[]): { width: number; height: number } {
  if (frames.length === 0) throw new Error("At least one frame is required.");
  for (const frame of frames) validateImage(frame);
...
export function composeOnionSkin(
  previousFrames: ImageBuffer[],
  currentFrame: ImageBuffer,
  nextFrames: ImageBuffer[],
  opacity = 0.35
): ImageBuffer {
  ...
  const applyGhost = (frame: ImageBuffer, color: [number, number, number], distance: number): void => {
  ...
  for (let offset = 0; offset < output.length; offset += 4) {
    blendPixel(
      ...
      currentFrame.data[offset + 2],
      currentFrame.data[offset + 3] / 255
    );
  }
  ...
  for (let pixel = 0; pixel < width * height; pixel++) {
    ...
    if (changed) {
      ...
      const luminance = Math.round(
        after.data[offset] * 0.2126 + after.data[offset + 1] * 0.7152 + after.data[offset + 2] * 0.0722
      );
