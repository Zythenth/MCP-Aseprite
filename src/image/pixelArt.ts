...
export function parseHexColor(input: string): RgbaColor {
  ...
  if (value.length === 3 || value.length === 4) value = [...value].map((part) => part + part).join("");
  if (value.length === 6) value += "FF";
  if (!/^[0-9a-fA-F]{8}$/.test(value)) throw new Error(`Invalid hex color: ${input}`);
  return {
...
function hslToRgb(hue: number, saturation: number, lightness: number, alpha = 255): RgbaColor {
  const h = ((hue % 360) + 360) % 360 / 360;
  const s = Math.max(0, Math.min(1, saturation));
  const l = Math.max(0, Math.min(1, lightness));
  if (s === 0) return { r: l * 255, g: l * 255, b: l * 255, a: alpha };
...
export function generatePaletteRamp(
  baseColor: string,
  steps: number,
...
export function analyzeImagePalette(image: ImageBuffer, nearDuplicateThreshold = 3): Record<string, unknown> {
  ...
  for (let offset = 0; offset < image.data.length; offset += 4) {
    ...
    if (image.data[offset + 3] === 0) transparentPixels++;
    else {
      if (!counts.has(color) && counts.size >= MAX_TRACKED_PALETTE_COLORS) {
        throw new Error(
          `Palette analysis supports at most ${MAX_TRACKED_PALETTE_COLORS} distinct opaque colors; `
  ...
  const nearDuplicates: Array<{ colorA: string; colorB: string; deltaE: number }> = [];
  for (let left = 0; left < duplicateCandidates.length; left++) {
    for (let right = left + 1; right < duplicateCandidates.length; right++) {
      const deltaE = deltaE2000(
        rgbToLab(parseHexColor(duplicateCandidates[left].hex)),
        rgbToLab(parseHexColor(duplicateCandidates[right].hex))
      );
  ...
  if (chromaticHues.length === 1) harmony = "monochromatic";
  else if (chromaticHues.length > 1) {
    const circularDistances: number[] = [];
    ...
    if (circularDistances.every((distance) => distance <= 35)) harmony = "analogous";
    else if (circularDistances.some((distance) => Math.abs(distance - 180) <= 25)) harmony = "complementary";
    else if (circularDistances.filter((distance) => Math.abs(distance - 120) <= 25).length >= 2) harmony = "triadic";
    else harmony = "mixed";
  }
  return {
...
function colorAt(image: ImageBuffer, x: number, y: number): number {
  const offset = (y * image.width + x) * 4;
  return image.data[offset] | image.data[offset + 1] << 8 | image.data[offset + 2] << 16 | image.data[offset + 3] << 24;
}

function alphaAt(image: ImageBuffer, x: number, y: number): number {
  return image.data[(y * image.width + x) * 4 + 3];
}

export function lintPixelArt(
  image: ImageBuffer,
  maxFindings = 200
): { findings: PixelArtFinding[]; summary: Record<string, number>; truncated: boolean; droppedFindings: number } {
  const findings: PixelArtFinding[] = [];
  let droppedFindings = 0;
  const add = (finding: PixelArtFinding): void => {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (alpha > 0) {
        ...
        for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) {
          ...
          if (colorAt(image, x + dx, y + dy) === ownColor) matchingNeighbors++;
        }
        if (matchingNeighbors === 0) add({
  ...
  for (let y = 0; y + 1 < height; y++) {
    ...
    while (runStart < width) {
      ...
      while (runEnd < width && colorAt(image, runEnd, y) === color && colorAt(image, runEnd, y + 1) === color) runEnd++;
  ...
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) if (alphaAt(image, x, y) > 0) {
  ...
  if (maxX >= minX && maxY >= minY && maxX - minX >= 4 && maxY - minY >= 4) {
    ...
    for (let y = minY; y <= maxY; y++) for (let x = minX; x <= maxX; x++) {
      ...
      if (edge) { edgeLight += light; edgeCount++; } else { innerLight += light; innerCount++; }
    }
    if (innerCount > 0 && edgeCount > 0 && innerLight / innerCount - edgeLight / edgeCount >= 24) add({
  ...
  for (let y = 0; y < height; y++) for (let x = 0; x < Math.floor(width / 2); x++) {
    if ((alphaAt(image, x, y) > 0) !== (alphaAt(image, width - 1 - x, y) > 0)) symmetryMismatches++;
  }
  if (symmetryPairs > 0 && symmetryMismatches > 0) add({
  ...
  for (let y = 0; y < height; y++) if (colorAt(image, 0, y) !== colorAt(image, width - 1, y)) horizontalSeams++;
  for (let x = 0; x < width; x++) if (colorAt(image, x, 0) !== colorAt(image, x, height - 1)) verticalSeams++;
  if (horizontalSeams > 0 || verticalSeams > 0) add({
  ...
  for (const finding of findings) summary[finding.rule] = (summary[finding.rule] ?? 0) + 1;
...
export function generateDitherPixels(
