...
export interface CheckerboardOptions {
  cellSize?: number;              // Tile size in pixels (default: 8)
  lightColor?: CheckerboardColor; // Default: 204 (#CCCCCC)
  darkColor?: CheckerboardColor;  // Default: 153 (#999999)
}
 ...
 */
export function normalizeCheckerColor(c: CheckerboardColor | undefined, defaultVal: number): RgbColor {
 ...
 */
function packRgba32(color: RgbColor, alpha = 255): number {
 ...
 */
export function applyCheckerboardBackdrop(
  srcData: Uint8Array,
  width: number,
  height: number,
  cellSizeOrOptions?: number | CheckerboardOptions,
  legacyLightGray?: number,
  legacyDarkGray?: number
): Uint8Array {
 ...
 */
export function createCheckerboardBuffer(
  width: number,
  height: number,
