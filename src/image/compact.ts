...
export interface CompactGridResult {
  palette: Record<string, string>; // Symbol -> #RRGGBBAA
  rows: string[];                  // 2D character matrix rows
  width: number;
  height: number;
  colorCount: number;
...
export interface CompactGridOptions {
  transparentSymbol?: string;      // Default: "."
  customAlphabet?: string;         // Default: A-Z, a-z, 0-9
  normalizeTransparency?: boolean; // Default: true (converts any alpha=0 to #00000000)
  includeFrequencies?: boolean;    // Default: false
}
...
function normalizeHexColor(hex: string, normalizeZeroAlpha = true): string {
...
export function buildCompactGrid(
  pixelHexArray: string[][],
  width?: number,
  height?: number,
  options?: CompactGridOptions
): CompactGridResult {
  ...
  const nonTransColors: Array<{ color: string; count: number }> = [];
  ...
  const result: CompactGridResult = {
    palette,
    rows,
    width: w,
    height: h,
 ...
 */
export function buildCompactGridFromRgba(
  rgba: Uint8Array,
  width: number,
  height: number,
  options?: CompactGridOptions
 ...
 */
export function decompressCompactGrid(compact: CompactGridResult): string[][] {
