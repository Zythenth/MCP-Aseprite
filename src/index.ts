...
async function main(): Promise<void> {
  ...
  const startBridge = async (): Promise<void> => {
    if (isShuttingDown || wsServer) return;
    ...
    try {
      ...
      state.setConnectionIssue(
        `The Aseprite bridge port ${config.port} is owned by another aseprite-mcp process. Stop the standalone process (for example start.ps1) and this MCP client will retry automatically.`
      );
