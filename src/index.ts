...
function isConnectionRefused(error: unknown): boolean {
  ...
  if ("code" in error && (error as NodeJS.ErrnoException).code === "ECONNREFUSED") return true;
...
async function main(): Promise<void> {
  ...
  let sharedBridge: SharedBridgeClient | null = null;
  ...
  const scheduleBridgeRetry = () => {
    ...
    retryTimer = setTimeout(() => {
      retryTimer = null;
      void startBridge().catch((retryError: unknown) => {
  ...
  const startBridge = async (): Promise<void> => {
    if (isShuttingDown || sharedBridge || bridgeStartInProgress) return;
    ...
    try {
      ...
      if (sharedBridge === peer) sharedBridge = null;
      ...
      if (isConnectionRefused(error) && Date.now() - lastDaemonSpawnAt >= DAEMON_SPAWN_COOLDOWN_MS) {
        ...
        state.setConnectionIssue(
          `Starting the persistent Aseprite bridge daemon on port ${config.port}; retrying automatically.`
        );
        logger.info(`Persistent bridge daemon launch requested (pid ${daemon.pid ?? "pending"}).`);
      } else {
        state.setConnectionIssue(
          `Could not join the persistent bridge on port ${config.port}; aseprite-mcp will keep retrying automatically.`
        );
