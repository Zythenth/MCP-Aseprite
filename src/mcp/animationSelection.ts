...
export interface AnimationPlayback {
  ...
  frameNumbers: number[];
  frames: Array<AnimationFrameSummary & { sequenceIndex: number }>;
  totalDurationMs: number;
  averageFps: number;
  variableTiming: boolean;
}

export function buildPlaybackFrameNumbers(
  fromFrame: number,
  toFrame: number,
  direction: AnimationDirection
): number[] {
  ...
  if (direction === "pingpong") return [...forward, ...interior.reverse()];
  if (direction === "pingpong_reverse") return [...forward].reverse().concat(interior);
...
export function resolveAnimationPlayback(
  ...
  const byNumber = new Map<number, AnimationFrameSummary>();
  ...
  let tag: AnimationTagSummary | undefined;
  if (input.tagName) {
    tag = inspection.tags.find((candidate) => candidate.name === input.tagName);
    if (!tag) throw new Error(`Animation tag not found: '${input.tagName}'.`);
  }
