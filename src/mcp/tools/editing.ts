...
export function registerEditingTools(
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const res = await dispatcher.send<any>("set_pixels", params, 15000);
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const res = await dispatcher.send<any>("set_pixel", params, 10000);
        if (typeof res.revision === "number") {
          stateTracker.setRevision(res.revision);
        }
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const res = await dispatcher.send<any>("erase_pixels", params, 10000);
        if (typeof res.revision === "number") {
          stateTracker.setRevision(res.revision);
        }
