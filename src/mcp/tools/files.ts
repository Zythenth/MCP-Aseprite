...
import { decodePngBase64Sync, decodePngBufferSync, type ImageBuffer } from "../../image/png.js";
...
function validateReferenceDimensions(width: number, height: number): { width: number; height: number } {
  if (
    ...
    throw new Error(
      `Reference image dimensions ${width}x${height} exceed the ${MAX_REFERENCE_DIMENSION}px / ${MAX_REFERENCE_PIXELS.toLocaleString()} pixel limit.`
    );
  }
  return { width, height };
}

function validatePngHeader(buffer: Buffer): { width: number; height: number } {
...
function validateJpegHeader(buffer: Buffer): { width: number; height: number } {
  ...
  while (offset < buffer.length) {
    ...
    if (startOfFrameMarkers.has(marker)) {
      if (segmentLength < 7) throw new Error("Reference JPEG has a truncated frame header.");
...
function validateWebpHeader(buffer: Buffer): { width: number; height: number } {
  ...
  while (offset + 8 <= buffer.length) {
    ...
    if (dataOffset + chunkSize > buffer.length) throw new Error("Reference WebP contains a truncated chunk.");
    if (chunkType === "VP8X") {
      if (chunkSize < 10) throw new Error("Reference WebP has a truncated VP8X header.");
      return validateReferenceDimensions(readUInt24LE(buffer, dataOffset + 4) + 1, readUInt24LE(buffer, dataOffset + 7) + 1);
    }
    if (chunkType === "VP8L") {
      if (chunkSize < 5 || buffer[dataOffset] !== 0x2f) throw new Error("Reference WebP has an invalid VP8L header.");
    ...
    if (chunkType === "VP8 ") {
      ...
      return validateReferenceDimensions(
...
function validateReferenceHeader(buffer: Buffer, extension: string): { width: number; height: number } {
  if (extension === ".png") return validatePngHeader(buffer);
  if (extension === ".jpg" || extension === ".jpeg") return validateJpegHeader(buffer);
  if (extension === ".webp") return validateWebpHeader(buffer);
  throw new Error(`Unsupported reference image extension '${extension}'.`);
}

function requireBaseName(fileName: string): string {
...
function resolveReferenceInput(params: { filePath?: string; directory?: string; fileName?: string }): string {
  ...
  if (!params.fileName) throw new Error("Provide filePath or fileName.");
...
function alphaSummary(image: ImageBuffer): { hasTransparency: boolean; transparentPixels: number; translucentPixels: number } {
...
function requireFileStem(value: string): string {
  const stem = requireBaseName(value);
  if (path.extname(stem) !== "") throw new Error("baseName must not include a file extension.");
  return stem;
}
...
export function registerFileTools(
  ...
  server.tool(
    "open_sprite",
    "Opens an existing sprite file (.ase, .aseprite, .png) from disk. Accepts relative (project-root based) or absolute path.",
    {
  ...
  server.tool(
    "save_sprite_as",
    "Saves the active sprite to a specific target file path on disk. Accepts relative (project-root based) or absolute path.",
    {
  ...
  server.tool(
    "save_project",
    "Safely saves the active document as an .aseprite project using an exact supplied file name or a deterministic descriptive name.",
    {
    ...
    async (params) => {
      try {
        ...
        if (params.filePath) {
          ...
          if (fileName) {
            ...
            const [status, tagResult] = await Promise.all([
              dispatcher.send<any>("aseprite_status", {}, 5_000),
              dispatcher.send<any>("list_tags", {}, 5_000),
            ]);
        ...
        const canonicalPath = validateSaveAsPath(
          targetPath,
        ...
        const result = await dispatcher.send<any>("save_sprite_as", {
  ...
  server.tool(
    "export_png",
    "Exports the active sprite or frame to a PNG image file on disk. Accepts relative or absolute path.",
    {
  ...
  server.tool(
    "export_sprite_sheet",
    "Exports a tag or explicit frame range as a horizontal, vertical, or grid PNG sheet, optionally compositing only selected non-group layers.",
    {
      outputPath: z.string().describe("Target .png file path (relative or absolute)"),
  ...
  server.tool(
    ...
    "Finds static reference images only inside an authorized directory or the configured project root. It never scans outside allowed roots.",
    {
        ...
        const scan = (directory: string, depth: number): void => {
          ...
          for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
              if (recursive && depth < maxDepth) scan(candidate, depth + 1);
              continue;
            }
            if (!entry.isFile() || !matchesName(entry.name)) continue;
            const extension = path.extname(entry.name).toLowerCase();
            if (!ALLOWED_REFERENCE_EXTENSIONS.includes(extension as typeof ALLOWED_REFERENCE_EXTENSIONS[number])) continue;
            try {
  ...
  server.tool(
    ...
    "Loads and analyzes a local reference image without switching or modifying the active Aseprite sprite.",
    {
      ...
      try {
        ...
        if (extension === ".png") {
          ...
          if (!stateTracker.getCapabilities().referenceImageDecode) {
            throw new Error(
              "The connected Aseprite bridge cannot decode JPEG/WebP references. Reinstall the bundled Lua bridge or use PNG."
            );
          }
          const result = await dispatcher.send<any>("load_reference_image", { filePath: canonicalPath }, 15_000);
  ...
  server.tool(
    "export_animation",
    "Exports a selected animation as a GIF, sprite sheet, PNG sequence, or single PNG. APNG is reported as unsupported unless a future bridge implements it.",
    {
    ...
    async (params: {
      ...
      try {
        ...
        if (params.format === "png_sequence") {
          if (params.outputPath) throw new Error("png_sequence uses outputDirectory/baseName, not outputPath.");
          ...
          try {
            ...
            if (!overwrite) {
              await Promise.all(completed.map(async (target) => {
                try { await fs.promises.unlink(target.outputPath); } catch { /* best-effort rollback */ }
              }));
            }
            throw error;
          }
        ...
        if (!params.outputPath) throw new Error(`outputPath is required for ${params.format}.`);
        ...
        if (params.format === "gif") {
          ...
          if (playback.frameNumbers.length > 64) throw new Error("GIF export is limited to 64 playback frames.");
          const outputPath = validateSaveAsPath(params.outputPath, overwrite, [".gif"], roots, true, projectRoot);
          const result = await dispatcher.send<any>("render_animation_gif", {
            outputPath,
            overwrite,
          ...
          return { content: [{ type: "text" as const, text: JSON.stringify({ ...result, format: params.format, playback }, null, 2) }] };
        }
        ...
        if (params.format === "png") {
          ...
          const result = await dispatcher.send<any>("export_png", {
            outputPath,
            frameNumber: sourceFrame,
            scale,
            overwrite,
          }, 15_000);
          return { content: [{ type: "text" as const, text: JSON.stringify({ ...result, format: params.format, playback, sourceFrame }, null, 2) }] };
        }
