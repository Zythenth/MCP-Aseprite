...
import { buildClusterTween, type PixelGrid } from "../../image/pixelMotion.js";
import { analyzeAnimationTemporalPure, type ContactPointInput } from "../../image/temporalAnalysis.js";
import { resolveAnimationPlayback, type AnimationInspection, type AnimationLayerInspection } from "../animationSelection.js";
import { buildDirectionalActionPlan, directionsFor, evaluateMirrorability, type DirectionalActionInput, type FacingDirection } from "../directionalActions.js";
...
function imageLayerNames(layers: AnimationLayerInspection[]): string[] {
  const names: string[] = [];
  const visit = (nodes: AnimationLayerInspection[]) => {
    for (const layer of nodes) {
      if (layer.isGroup && layer.children) visit(layer.children);
      else if (layer.isImage) names.push(layer.name);
    }
  };
  visit(layers);
  const duplicates = names.filter((name, index) => names.indexOf(name) !== index);
  if (duplicates.length) throw new Error(`Mirroring requires unique image-layer names; duplicated name '${duplicates[0]}' was found.`);
  return names;
}

function validatePixelGrid(result: unknown, width: number, height: number, layerName: string): PixelGrid {
  if (!result || typeof result !== "object") throw new Error(`Aseprite did not return a pixel grid for layer '${layerName}'.`);
...
function gridPixels(grid: PixelGrid): Array<{ x: number; y: number; color: string }> {
  return grid.flatMap((row, y) => row.map((color, x) => ({ x, y, color })));
}

function keyPoseOutputIndices(keyPoseCount: number, frameCount: number): number[] {
  if (keyPoseCount > frameCount) throw new Error(`Cannot place ${keyPoseCount} key poses into ${frameCount} output frames.`);
  const positions = Array.from({ length: keyPoseCount }, (_, index) => Math.round(index * (frameCount - 1) / (keyPoseCount - 1)));
  if (new Set(positions).size !== positions.length) throw new Error("Key poses collapse onto the same output frame; increase the requested frame count.");
  return positions;
}

async function inspect(dispatcher: CommandDispatcher, state: BridgeState): Promise<AnimationInspection> {
  const result = await dispatcher.send<AnimationInspection>("inspect_animation", {}, 10_000);
...
function planFromArgs(args: DirectionalActionInput): ReturnType<typeof buildDirectionalActionPlan> {
  return buildDirectionalActionPlan(args);
}

function tagNamesMissing(inspection: AnimationInspection, names: string[]): string[] {
  const present = new Set(inspection.tags.map((tag) => tag.name));
  return names.filter((name) => !present.has(name));
}

function ensureSourceFrame(inspection: AnimationInspection, frameNumber: number): void {
...
function contactsForTag(contacts: Array<z.infer<typeof contactSchema>>, tagName: string): ContactPointInput[] {
  return contacts
    .filter((contact) => contact.tagName === tagName)
    .map(({ tagName: _tagName, ...contact }) => contact);
}

function groundContactsForTag(contacts: Array<z.infer<typeof groundContactSchema>>, tagName: string): Array<z.infer<typeof groundContactSchema>> {
  return contacts.filter((contact) => contact.tagName === tagName);
}

function armSwingForTag(swings: Array<z.infer<typeof armSwingSchema>>, tagName: string): Array<z.infer<typeof armSwingSchema>> {
  return swings.filter((swing) => swing.tagName === tagName);
}

function evaluateArmSwing(swing: z.infer<typeof armSwingSchema>): { axis: "x" | "y"; pairedSteps: number; oppositeSteps: number; passed: boolean; reason: string } {
  ...
  for (let index = 1; index < frameNumbers.length; index += 1) {
    ...
    pairedSteps += 1;
    if (leftDelta * rightDelta < 0) oppositeSteps += 1;
  }
  return {
    axis: swing.axis,
    pairedSteps,
...
async function rollbackTimelineCreation(
  dispatcher: CommandDispatcher,
  createdTagNames: string[],
  createdFrameNumbers: number[],
): Promise<string[]> {
...
export function registerDirectionalActionTools(server: McpServer, dispatcher: CommandDispatcher, state: BridgeState): void {
  server.tool(
    "plan_directional_animation",
    "Builds a directional pixel-art action plan for walk, run, idle, attack, hit, death, dash, cast, or interaction. It produces per-direction tags, pose choreography, timing, events for engine export, and explicit QA checks; it never invents or paints character pixels.",
    actionInputSchema,
    async (args) => {
      try {
        return { content: [{ type: "text" as const, text: JSON.stringify({ success: true, plan: planFromArgs(args) }, null, 2) }] };
      } catch (error) {
  ...
  server.tool(
    "create_directional_animation_timeline",
    "Creates a complete 4- or 8-direction timeline by duplicating a supplied pose-base frame, applying the requested timing, and creating one tag per direction. The copied frames are deliberate editable pose slots: draw the action poses before QA; the tool does not pretend an unchanged pose is a finished walk, run, or attack.",
    { ...actionInputSchema, sourceFrame: z.number().int().positive() },
    async (args) => {
      ...
      try {
        ...
        if (conflicts.length) throw new Error(`Refusing to overwrite existing tag '${conflicts[0].name}'. Choose another tagPrefix.`);

        const createdTags: Array<{ direction: string; name: string; fromFrame: number; toFrame: number; events: Array<{ name: string; frameNumber: number; payload?: string }> }> = [];
        let sourceFrameToCopy = args.sourceFrame;
        for (const tag of plan.tags) {
          const frames: number[] = [];
          for (let index = 0; index < plan.framesPerDirection; index += 1) {
            const duplicate = await dispatcher.send<{ newFrameNumber?: unknown; newFrame?: unknown }>("duplicate_frame", { frameNumber: sourceFrameToCopy }, 10_000);
            const frameNumber = duplicate.newFrameNumber ?? duplicate.newFrame;
            if (typeof frameNumber !== "number" || !Number.isInteger(frameNumber) || frameNumber < 1) throw new Error("Aseprite did not return a newly duplicated frame number.");
            ...
            if (frameNumber === sourceFrameToCopy) sourceFrameToCopy += 1;
          }
        ...
        if (missing.length) throw new Error(`Aseprite did not retain created tag '${missing[0]}'.`);
        ...
        if (!createdTagNames.length && !createdFrameNumbers.length) return bridgeToolError(error);
  ...
  server.tool(
    "generate_directional_animation_from_key_poses",
    "Generates an editable 4- or 8-direction action cycle from two or more artist-drawn key poses per direction. It creates independent output frames, uses crisp same-color cluster interpolation only between supplied poses, applies timing and tags, and returns engine events. It never claims that interpolation replaces visual pose review.",
    { ...actionInputSchema, keyPoses: z.array(keyPoseSchema).min(4).max(8) },
    async (args) => {
      const createdTagNames: string[] = [];
      const createdFrameNumbers: number[] = [];
      try {
        ...
        const suppliedKeyPoses = new Map<FacingDirection, number[]>();
        for (const keyPose of args.keyPoses) {
          if (!expectedDirections.includes(keyPose.direction)) throw new Error(`Direction '${keyPose.direction}' is not available in ${args.directions}-direction mode.`);
          if (suppliedKeyPoses.has(keyPose.direction)) throw new Error(`Key poses for direction '${keyPose.direction}' were supplied more than once.`);
          if (keyPose.frameNumbers.length > args.frames) throw new Error(`Direction '${keyPose.direction}' supplies more key poses than the requested ${args.frames} output frames.`);
          if (new Set(keyPose.frameNumbers).size !== keyPose.frameNumbers.length) throw new Error(`Key poses for direction '${keyPose.direction}' must use distinct source frames.`);
          suppliedKeyPoses.set(keyPose.direction, [...keyPose.frameNumbers]);
        }
        for (const direction of expectedDirections) {
          if (!suppliedKeyPoses.has(direction)) throw new Error(`Missing artist-drawn key poses for direction '${direction}'.`);
        }
        ...
        for (const [direction, frameNumbers] of suppliedKeyPoses) {
          for (const frameNumber of frameNumbers) {
            if (!existingFrames.has(frameNumber)) throw new Error(`Key pose frame ${frameNumber} for direction '${direction}' does not exist in the active sprite.`);
          }
        }
        ...
        if (!layerNames.length) throw new Error("No image layers are available for key-pose generation.");

        const sourceGrids = new Map<FacingDirection, Array<Map<string, PixelGrid>>>();
        for (const direction of expectedDirections) {
          ...
          const grids = await Promise.all(frames.map(async (frameNumber) => {
            const pairs = await Promise.all(layerNames.map(async (layerName) => {
              ...
              return [layerName, validatePixelGrid(result, before.width, before.height, layerName)] as const;
            }));
            return new Map<string, PixelGrid>(pairs);
          }));
        ...
        let insertAfterFrame: number = lastExistingFrame;
        const generatedTags: Array<{ direction: FacingDirection; name: string; fromFrame: number; toFrame: number; events: Array<{ name: string; frameNumber: number; payload?: string }> }> = [];
        const outputByDirection = new Map<FacingDirection, number[]>();
        for (const tag of plan.tags) {
          const frames: number[] = [];
          for (let index = 0; index < plan.framesPerDirection; index += 1) {
            const created: { createdFrameNumber?: unknown; frameNumber?: unknown } = await dispatcher.send("create_frame", {
              afterFrame: insertAfterFrame,
              duration: plan.frameDurationMs,
            }, 10_000);
            const returnedFrameNumber: unknown = created.createdFrameNumber ?? created.frameNumber;
            if (typeof returnedFrameNumber !== "number" || !Number.isInteger(returnedFrameNumber) || returnedFrameNumber < 1) throw new Error("Aseprite did not return a generated frame number.");
        ...
        const generatedInBetweens: Array<{ direction: FacingDirection; frameNumber: number; betweenKeyPoseIndices: [number, number]; movedClusters: number; unmatchedClusters: number }> = [];
        for (const tag of plan.tags) {
        ...
        if (missing.length) throw new Error(`Aseprite did not retain generated tag '${missing[0]}'.`);
        return { content: [{ type: "text" as const, text: JSON.stringify({
          success: true,
          action: plan.action,
          generatedTags,
  ...
  server.tool(
    ...
    async (args) => {
      try {
        ...
        if (missing.length) throw new Error(`Missing required directional tag '${missing[0]}'.`);
        const perDirection: Array<Record<string, unknown>> = [];
        for (const tag of plan.tags) {
          ...
          const frames = await Promise.all(playback.frameNumbers.map(async (frameNumber) => {
            const canvas = await dispatcher.send<any>("get_canvas", { frameIndex: frameNumber }, 10_000);
            if (typeof canvas.revision === "number") state.setRevision(canvas.revision);
          ...
          const centers = temporal.metrics.perFrameMetrics.map((metric) => metric.visualCenter?.y).filter((center): center is number => center !== null);
  ...
  server.tool(
    ...
    async (args) => ({ content: [{ type: "text" as const, text: JSON.stringify({ success: true, decision: evaluateMirrorability(args) }, null, 2) }] })
  );

  server.tool(
    "mirror_directional_animation",
    ...
    async (args) => {
      ...
      let sourceTagForRollback: AnimationInspection["tags"][number] | null = null;
      let targetTagCreated = false;
      try {
        const decision = evaluateMirrorability(args);
        if (!decision.mirrorable) throw new Error(`Mirroring blocked: ${decision.blockingReasons.join(" ")}`);
        const before = await inspect(dispatcher, state);
        if (before.tags.some((tag) => tag.name === args.targetTagName)) throw new Error(`Refusing to overwrite existing tag '${args.targetTagName}'.`);
        const sourceTag = before.tags.find((tag) => tag.name === args.sourceTagName);
        if (!sourceTag) throw new Error(`Source tag '${args.sourceTagName}' was not found.`);
        ...
        const layerNames = imageLayerNames(before.layers);
        if (!layerNames.length) throw new Error("No image layers are available to mirror.");
        ...
        const sourcePixels = await Promise.all(sourceFrames.map(async (sourceFrame) => ({
          ...
          layers: await Promise.all(layerNames.map(async (layerName) => {
            const gridResult = await dispatcher.send<{ width?: unknown; height?: unknown; grid?: unknown }>("get_pixel_grid", { frameIndex: sourceFrame.frameNumber, layerName, format: "hex" }, 10_000);
            ...
            const pixels = gridResult.grid.flatMap((row, y) => {
              if (!Array.isArray(row) || row.length !== before.width) throw new Error(`Aseprite returned an invalid pixel row for layer '${layerName}'.`);
              return row.map((color, x) => {
                if (typeof color !== "string") throw new Error(`Aseprite returned an invalid pixel color for layer '${layerName}'.`);
        ...
        for (const sourceFrame of sourceFrames) {
          const created = await dispatcher.send<{ createdFrameNumber?: unknown; frameNumber?: unknown }>("create_frame", {
            afterFrame: insertAfterFrame,
            duration: sourceFrame.durationMs,
          }, 10_000);
          const targetFrame = created.createdFrameNumber ?? created.frameNumber;
          if (typeof targetFrame !== "number" || !Number.isInteger(targetFrame) || targetFrame < 1) throw new Error("Aseprite did not return a newly created mirror frame number.");
        ...
        const after = await inspect(dispatcher, state);
        if (!after.tags.some((tag) => tag.name === args.targetTagName)) throw new Error(`Aseprite did not retain mirror tag '${args.targetTagName}'.`);
        const sourceTagAfter = after.tags.find((tag) => tag.name === args.sourceTagName);
        if (!sourceTagAfter) throw new Error(`Aseprite did not retain source tag '${args.sourceTagName}' while mirroring.`);
        return { content: [{ type: "text" as const, text: JSON.stringify({
          success: true,
