...
import type { AnimationInspection } from "../animationSelection.js";
import { bridgeToolError } from "./common.js";

const MAX_BATCH_FILES = 256;

async function findSpriteFiles(directory: string): Promise<string[]> {
  const result: string[] = [];
  async function visit(current: string): Promise<void> {
    for (const entry of await fs.promises.readdir(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await visit(candidate);
        ...
        if (result.length > MAX_BATCH_FILES) throw new Error(`Batch export is limited to ${MAX_BATCH_FILES} source sprites.`);
      }
    }
  }
...
function safeRelativeStem(inputDirectory: string, sourcePath: string): string {
  const relative = path.relative(inputDirectory, sourcePath);
  const noExtension = relative.slice(0, -path.extname(relative).length);
  if (!noExtension || noExtension.startsWith("..") || path.isAbsolute(noExtension)) throw new Error(`Invalid batch source path: ${sourcePath}`);
  return noExtension;
}

export function registerBatchExportTools(server: McpServer, dispatcher: CommandDispatcher): void {
  server.tool(
    "batch_export_sprites",
    "Exports every .ase/.aseprite under an approved directory as PNG frames or spritesheets at one or more integer scales. Existing files are never overwritten unless explicitly requested, and the original saved document is restored.",
    {
    ...
    async (args) => {
      try {
        ...
        const originalStatus = await dispatcher.send<{ filename?: string; hasActiveSprite?: boolean }>("aseprite_status", {}, 10_000);
        ...
        if (!sources.length) throw new Error("No .ase or .aseprite files found in inputDirectory.");
        const planned: Array<{ sourcePath: string; scale: number; outputPath: string }> = [];
        for (const sourcePath of sources) {
          ...
          for (const scale of [...new Set(args.scales)].sort((a, b) => a - b)) {
            ...
            if (!isPathWithinRoots(outputPath, roots)) throw new Error(`Batch output escapes configured roots: ${outputPath}`);
            planned.push({ sourcePath, scale, outputPath });
          }
        }
        if (!args.overwrite) {
          const conflicts = planned.filter((item) => fs.existsSync(item.outputPath));
          if (conflicts.length) throw new Error(`Refusing to overwrite ${conflicts.length} existing batch output(s), starting with ${conflicts[0].outputPath}.`);
        }
        const completed: Array<{ sourcePath: string; outputPath: string; scale: number; frames: number }> = [];
        try {
          for (const sourcePath of sources) {
            await dispatcher.send("open_sprite", { filePath: sourcePath }, 20_000);
            const info = await dispatcher.send<{ frames?: Array<{ frameNumber: number }> }>("get_sprite_info", {}, 10_000);
          ...
          if (originalPath) await dispatcher.send("open_sprite", { filePath: originalPath }, 20_000);
        }
