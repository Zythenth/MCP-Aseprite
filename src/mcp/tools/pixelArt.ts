...
async function fetchCanvas(
  ...
  state: BridgeState,
  params: { frameNumber?: number; layerName?: string }
) {
...
export function registerPixelArtTools(
  ...
  server.tool(
    "lint_pixel_art",
    "Runs deterministic pixel-art checks for orphan pixels, one-pixel gaps, banding, possible pillow shading, symmetry drift, and tile seams. Heuristic findings include confidence and are never applied automatically.",
    {
    ...
    async (params) => {
      try {
        ...
        const visibleSummary: Record<string, number> = {};
  ...
  server.tool(
    "analyze_palette",
    "Analyzes colors actually used in a rendered frame, including frequency, inferred harmony, and perceptually near-duplicate colors using CIEDE2000.",
    {
      frameNumber: z.number().int().positive().optional(),
      layerName: z.string().optional(),
  ...
  server.tool(
    "find_perceptual_palette_color",
    "Finds the nearest active-palette entry using CIEDE2000 rather than raw RGB distance.",
    {
    ...
    async (params) => {
      try {
        ...
        const response = await dispatcher.send<any>("get_palette", {}, 5_000);
        const colors = Array.isArray(response.colors) ? response.colors : [];
        const candidates = colors
          .map((entry: any) => ({ ...entry, parsed: parseHexColor(String(entry.hex)) }))
          .filter((entry: any) => params.includeTransparent || entry.parsed.a > 0);
        if (candidates.length === 0) throw new Error("The active palette has no eligible colors.");
        const ranked = candidates.map((entry: any) => ({
          index: entry.index,
          hex: entry.hex,
          deltaE: deltaE2000(targetLab, rgbToLab(entry.parsed)) + Math.abs(target.a - entry.parsed.a) / 255 * 100,
        })).sort((left: any, right: any) => left.deltaE - right.deltaE);
        return {
          content: [{ type: "text" as const, text: JSON.stringify({
            target: params.color,
            match: ranked[0],
  ...
  server.tool(
    "generate_palette_ramp",
    "Generates a deterministic lightness ramp around a base color with optional opposing shadow/highlight hue shifts. It does not modify the sprite.",
    {
      baseColor: z.string(),
    ...
    async (params) => {
      try {
        ...
        const colors = generatePaletteRamp(
          params.baseColor,
          params.steps ?? 5,
          params.shadowLightness ?? 0.18,
          params.highlightLightness ?? 0.82,
          params.hueShift ?? 12
        );
        return { content: [{ type: "text" as const, text: JSON.stringify({ colors }, null, 2) }] };
      } catch (error) {
        return bridgeToolError(error);
      }
    }
  ...
  server.tool(
    ...
    "Applies a bounded 2x2, 4x4, or 8x8 Bayer dither as one atomic set_pixels operation. Returns exact bounds and an optional updated preview.",
    {
      region: z.object({
    ...
    async (params) => {
      try {
        ...
        const result = await dispatcher.send<any>("set_pixels", {
          pixels,
          layerName: params.layerName,
